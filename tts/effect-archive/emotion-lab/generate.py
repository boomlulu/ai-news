...
def build_jobs(tax, only):
    """展开成 [(emo, var)] 列表，可按 only(情绪id 集合) 过滤。"""
    jobs = []
...
def write_scoring(outdir, rows, round_name):
    ...
    lines.append("")
    lines.append("| 序号 | 情绪 | 变体 | 文件 | instruct | 情绪契合(×2) | 自然 | 音色 | 清晰 | 可用 | 总分 | 最佳√ | 备注 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    last_emo = None
    for r in rows:
...
def main():
    ...
    ok_n = 0
    fail_n = 0
    for i, (emo, var) in enumerate(jobs, 1):
        fname = f"{emo['id']}_{safe_name(emo['name'])}__{var['v']}__{safe_name(var['slug'])}.mp3"
        ...
        rows.append({
            ...
            "fname": fname, "instruct": var["instruct"], "ok": ok,
        })

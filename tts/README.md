...
### ④ 口播停顿/杂音三连坑 —— 短段过量生成 / RMS 裁剪削音 / 停顿叠加（已用 `--unit smart` 治本）

把口播稿切成短碎片逐段独立合成、再在波形层拼接/裁剪，会连环出三个 bug：
1. **短段过量生成杂音**：单元文本远短于 zero_shot 参考音文本（`prompt_text` 37 字）时，CosyVoice2 报 `synthesis text too short than prompt text`，在短句尾部多生成一段无意义低能量呼吸/杂音（whisper 听成「怪鬼」，RMS≈−29 dB）。典型触发：`接下来是快读。`（7 字）单独成段。
2. **RMS 能量裁剪削软音**：为控停顿用能量门限（−28 dB）裁段尾静音，会把句末软音节的自然衰减一起削掉（「次方」念成「次」）。
3. **停顿叠加 / 忽长忽短**：不裁则每个单元自带不定长尾静音 ＋ 人工插入静音叠加 → 停顿过长不一致；逐逗号插静音则发碎。

**治本 = `--unit smart`（config `cosyvoice.synth_unit: "smart"` 已设为默认）：**
- 合成单元提升到**条目/整段级**（`min_unit_chars=22`、`target_max_chars=140`，只在 。！？；切，**绝不按逗号切**）→ 没有过短段，不触发过量生成。
- 端点归一化用 **silero-vad**（不是 RMS）：裁到 `[首 speech − pre_roll, 末 speech + tail_keep(160 ms)]` ＋ 短淡入淡出 → 句末软音完整保留、尾静音归一化到固定值。
- 条目间插固定停顿 `item_pause_ms`（默认 420）；实际听感停顿 ≈ `tail_keep + item_pause ≈ 580 ms`，一致；条目内逗号/句号停顿交给模型自带韵律。
- **`transformers==4.51.3` 锁定后长输入不 runaway**（实测 100 字整段 0.20 s/字），所以旧的「≤32 字切碎防 runaway」是多余防御，smart 直接绕过。
- 旋钮（config `cosyvoice` 或同名 CLI）：`synth_unit / min_unit_chars / target_max_chars / item_pause_ms / within_pause_ms / tail_keep_ms / pre_roll_ms`；CLI `--unit smart --item-pause --within-pause --tail-keep --min-unit --target-max`。新依赖 **`silero-vad`**（venv 已装）。
...
# 3) 合成音频（smart 停顿管线 + 克隆音色 my_voice_zh；CosyVoice 未装自动 fallback macsay）
tts/.venv/bin/python -m tts.tts_service --text-path tts/samples/daily-ai-news-${DATE}-broadcast.txt --out tts/samples/daily-ai-news-${DATE}-broadcast.mp3 --provider cosyvoice --voice my_voice_zh --unit smart
```

`daily-ai-news-<date>-broadcast.mp3` 会被每天 07:00 的 `ainews-auto-commit-push.sh` 一并提交入库（音频要入 git）。注意第 3 步必须用 venv python（`tts/.venv/bin/python`），系统 `python3`(3.9) 没装 cosyvoice 会 fallback 到 macsay。
